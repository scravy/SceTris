...
-->
	...
	<!--	<input type="hidden" name="~formType" value="de.fu.scetris.web.forms.SearchUser"/>
		<input type="hidden" name="~formName" value="foobar"/>	
	-->
		<xsl:call-template name="input-text">
			<xsl:with-param name="id" select="'pattern'" />
			<xsl:with-param name="label" select="'Searchterm'" />
		</xsl:call-template>

...
<xsl:transform version="1.0"
	...
	<xsl:template name="input-options">
		<xsl:param name="id" />
		<xsl:param name="name">
		...
		<xsl:param name="error-message">
			<xsl:choose>
				...
				<xsl:when test="$error-type = 'RequiredFieldMissingException'">
					<xsl:choose>
						<xsl:when test="$mod-lang/error-missing/*[local-name() = $id]">
							<xsl:value-of select="$mod-lang/error-missing/*[local-name() = $id]" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:text>required, but missing</xsl:text>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
			</xsl:choose>
		...
		<xsl:param name="class">
			...
			<xsl:choose>
				...
				<xsl:when test="$error-type = 'RequiredFieldMissingException'">
					<xsl:text> errorneous missing</xsl:text>
				</xsl:when>
			</xsl:choose>
		</xsl:param>
		
		<div class="options {$class}">
		<p class="optionsLabel">
		...
		<ul class="optionsBox">
			...
			<li>
				<xsl:if test="not(@s = '1')">
					<input type="checkbox" id="{generate-id(.)}" name="{$name}" value="{@k}" />
				</xsl:if>
				<xsl:if test="@s = '1'">
					<input type="checkbox" id="{generate-id(.)}" name="{$name}" value="{@k}" checked="checked" />
				</xsl:if>

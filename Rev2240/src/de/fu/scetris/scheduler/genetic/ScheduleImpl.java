...
public class ScheduleImpl implements Schedule, Comparable<Schedule> {
	 ...
	 */
	public ScheduleImpl(final Configuration configuration) {
	...
	public String printSchedule() throws DatabaseException {
		...
		for (int i = 0; i < configuration.numberOfDays; ++i) {
			longestDay = Math.max(longestDay, configuration.timeSlotsPerDay[i]);
			lineBreak = lineBreak.concat("-----------+");
			border = border.concat("           |");
			dayHeader = dayHeader.substring(0, 12 * configuration.numberOfDays + 1);
		}
		...
		for (int i = 0; i < roomTimeCourseSlotList.size(); ++i) {
			...
			new String();
			result = result.concat("| R" + String.format("%04d", roomTimeCourseSlotList.get(i).getRoom().getId())
					+ " ");
			...
			for (int j = 0; j < longestDay; ++j) {
				...
				for (int k = 0; k < configuration.numberOfDays; ++k) {

					if (configuration.timeSlotsPerDay[k] < j) {
						result = result.concat("|           ");
						continue;
					}
					...
					offset += configuration.timeSlotsPerDay[k];
				}

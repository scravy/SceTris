...
public class Installer {
    ...
    public static void main(String[] args) {
        BasicConfigurator.configure(new ConsoleAppender(new SimpleLayout(), "System.err"));
        ...
        try {
            for (int i = 0; i < args.length; i++) {
                if (args[i].equals("--help") || args[i].equals("-h")) {
                    ...
                    debug = true;
                } else if (args[i].equals("--privilegesXml")) {
                    privilegesXml = args[++i];
                }
            }
        ...
        try {
            ...
            /* IF GIVEN THE PRIVILEGES WILL BE SET */
            if (privilegesXml != null) {
                try {
                    Document privilegesFile = XmlHelper.parse(new File(privilegesXml));
                    manager.installPrivileges(privilegesFile);
                    System.out.println("INFO - Installing privileges from " + privilegesXml);
                } catch (IOException ioException) {
                    System.err.println("Could not open file " + privilegesXml);
                    ioException.printStackTrace();
                } catch (SAXException xmlException) {
                    System.err.println("Malformed xml file");
                    xmlException.printStackTrace();
                } catch (ParserConfigurationException seriousException) {
                    System.err.println("serious configuration failure");
                    seriousException.printStackTrace();
                } catch (Exception e) {
                    System.err.println("something not nice happended, maybe a database exception");
                    e.printStackTrace();
                }
            }

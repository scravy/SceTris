...
public class ScetrisServlet extends GenericWeavlet<RelationManager> {
    ...
    protected void beforeAction(final Module<RelationManager> $module,
        ...
        if ($currentUser instanceof Person) {
            ...
            List<Privilege> $privs = new LinkedList<Privilege>();
            // $privs.add($currentPerson.objectOfPersonHasRole());
            $privs.addAll($currentPerson.objectsOfPersonHasPrivilege());
            for (Role $priv : $currentPerson.objectsOfPersonHasRole()) {
                $privs.addAll($priv.objectsOfRoleImpliesPrivilege());
            }

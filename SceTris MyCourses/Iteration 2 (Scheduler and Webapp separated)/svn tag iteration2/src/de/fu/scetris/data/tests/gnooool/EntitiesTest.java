...
import de.fu.scetris.data.tests.TestNames;
 ...
 * @since Iteration3
 */
public class EntitiesTest extends de.fu.weave.tests.DatabaseTestSkeleton {
	...
	public void stressTest() throws DatabaseException {
		...
		TestNames names = new TestNames(rand);
		for (int i = 0; i < DEPARTMENTS; i++) {
			TestNames.Name n1 = names.nextName();
			department[i] =
		...
		for (int i = 0; i < BUILDINGS; i++) {
			...
			if (i % 3 == 0) {
				TestNames.Name n2 = names.nextName();
		...
		for (int i = 0; i < PERSONS; i++) {
			persons[i] =
					manager.newPerson(TestNames.firstNames[rand.nextInt(TestNames.firstNames.length)],
										 TestNames.lastNames[rand.nextInt(TestNames.lastNames.length)],
										 "user" + i,
										 md5(i + "pw" + Math.random()));
			if ((i % 42 == 7) || (i % 7 == 0)) {
				persons[i].setAdditionalNames(TestNames.firstNames[rand
						.nextInt(TestNames.firstNames.length)]);
			}
			persons[i].create();
		...
		for (int i = 0; i < PERSONS; i += 3) {
			persons[i].setAdditionalNames(TestNames.firstNames[rand.nextInt(TestNames.firstNames.length)]);

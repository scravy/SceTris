...
public class XmlWriter {
	...
	private void findNamespaces(final Element $element) {
		...
		if (!$namespaces.containsKey(namespace)) createNamespace(namespace);
	...
	private void findNamespaces(final NamedNodeMap attrs) {
		for (int i = 0; i < attrs.getLength(); i++) {
			...
			if (namespace != null) if (!$namespaces.containsKey(namespace)) createNamespace(namespace);
		}
	}

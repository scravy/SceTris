...
/**
 * A command line tool for setting up the database.
 */
...
public class Installer {
	...
	public static void main(String[] args) {
		...
		if (authenticate || Authenticate) if (c == null)
		...
		if (fullInstall) {
			...
			if (verbose) System.out.println("Doing a complete install.");
			...
			try {
				...
				if (verbose) System.out.println("Dropping database " + database + " if it exists");
				query = "DROP DATABASE IF EXISTS " + escape(database) + ";";
				if (debug) System.out.println("[debug] " + query);
				connection.createStatement().execute(query);

				if (username != null) {
					if (verbose) System.out.println("Dropping role " + username + " if it exists");
					query = "DROP ROLE IF EXISTS " + escape(username) + ";";
					if (debug) System.out.println("[debug] " + query);
					connection.createStatement().execute(query);
					if (verbose) System.out.println("Creating role " + username);
					query =
				...
				if (verbose) System.out.println("Creating " + database + '@' + hostname + " with owner "
						+ username);
				...
				if (debug) System.err.println("[debug] Query was " + query);
				System.exit(1);
			} finally {
			...
			if (verbose) System.out.println("Connection to production-db to configure languages.");
			...
			try {
				...
				if (verbose) System.out.println("Dropping language plpgsql if it exists");
				query = "DROP LANGUAGE IF EXISTS plpgsql CASCADE;";
				if (debug) System.out.println("[debug] " + query);
				connection.createStatement().execute(query);
				if (verbose) System.out.println("Creating language plpgsql");
				query = "CREATE LANGUAGE plpgsql;";
				if (debug) System.out.println("[debug] " + query);
				connection.createStatement().execute(query);
				if (verbose) System.out.println("Setting owner of plpgsql");
		...
		try {
			manager.install();
			String query = "";
			query = "INSERT INTO \"scetris\".\"Person\" (first_name, last_name, login_name, login_password, is_superuser) VALUES ('"
					+ admin + "','" + admin + "','" + admin + "','" + md5(admin_pw) + "',TRUE);";
			if (debug) System.out.println("[debug] " + query);
			try {

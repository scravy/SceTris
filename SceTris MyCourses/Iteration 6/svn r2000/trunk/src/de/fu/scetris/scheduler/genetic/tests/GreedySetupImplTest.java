...
import de.fu.scetris.scheduler.genetic.HardFitnessCalculationImpl;
import de.fu.scetris.scheduler.genetic.ScheduleImpl;
import de.fu.scetris.scheduler.genetic.SoftFitnessCalculationImpl;
...
public class GreedySetupImplTest extends SchedulerTest {
	 ...
	 *                 feature constraints.
	 * 
	 * @postcondition: a course with room and time slot preference is actually
	 *                 scheduled in the specified room at the specified time.
	 * 
	 * @postcondition: there are no gaps in the scheduled timetable because
	 *                 every course has the duration of two time slots.
	 * 
	 * @postcondition: there must not be more then one course at the same time
	 *                 in the same room.
	 * 
	 * @postcondition: the hard and soft fitness of the initialized schedule is
	 *                 1.0
	 */
	@Test
	public void testInitialize() throws Exception {
		...
		new HardFitnessCalculationImpl().calculateHardFitness(schedule,
				configuration);
		new SoftFitnessCalculationImpl().calculateSoftFitness(schedule,
				configuration);

/* PersonPrefersTimeslot.java / 2011-02-01+01:00
...
@de.fu.weave.orm.annotation.Relation(name = "\"scetris\".\"personPrefersTimeslot\"", requiredSqlCols = {"priority"})
...
public class PersonPrefersTimeslot extends de.fu.weave.orm.GenericRelationship {
	...
	PersonPrefersTimeslot(RelationManager manager, java.sql.ResultSet result) throws de.fu.weave.orm.DatabaseException {
		this.manager = manager;
		timekey(true);
		try {
			ref_user = result.getInt("user");
	...
	@Override
	public PersonPrefersTimeslot create() throws de.fu.weave.orm.DatabaseException {
	...
	public void pushChanges() throws de.fu.weave.orm.DatabaseException {
		...
		try {
			...
			if (rowsUpdated == 0) {
				throw new de.fu.weave.orm.OutdatedRecordException("\"scetris\".\"personPrefersTimeslot\"" + '#' + id());
			}
			_timekey = newTimekey;
		} catch (java.sql.SQLException e) {
			throw new de.fu.weave.orm.DatabaseException(query, e);
	 ...
	 * @since Iteration2
	 */
	public PersonPrefersTimeslot setUser(Person value) {
	 ...
	 * @since Iteration2
	 */
	public PersonPrefersTimeslot setTimeslot(Timeslot value) {
		if (manager.isNull(value)) {
...
@Override
	 ...
	 * @since Iteration2
	 */
	public PersonPrefersTimeslot setPriority(int value) {
		if (manager.isNull(value)) {

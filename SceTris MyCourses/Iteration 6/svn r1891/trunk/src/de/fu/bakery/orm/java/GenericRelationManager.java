...
import de.fu.bakery.orm.java.filters.All;
import de.fu.bakery.orm.java.filters.Any;
...
abstract public class GenericRelationManager implements RelationManager {
	...
	public static void extractSortFilters(final Filter[] $filters, final List<Sort> $target) {
		for (Filter $filter : $filters) {
			if ($filter instanceof Sort) {
				...
				extractSortFilters(((All) $filter).filters, $target);
			}
		}
	}
	...
	public static String makeWhereString(final Filter[] filters) {
		...
		for (Filter filter : filters) {
			if ((filter == null) || (filter instanceof Sort)) {
				continue;
			}
	...
	final Map<String,ResultSet> $queryCache = new TreeMap<String,ResultSet>();

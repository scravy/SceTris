...
public class Admin extends FriggModule<RelationManager> {
    ...
    public void schedulerPanel(final String[] $path, final SchedulingControl $form,
        ...
        if(schedulerManager.isRunning()) {
	        ...
	        if($path.length > 0) {
	        	...
	        	if($ps.size() > 0) {
	        		...
	        		List<CourseInstance> $x_ci = manager().getCourseInstance(0, 1, eq(CourseInstance.Program, $p.getId()));
	        		if($x_ci.size() > 0) {
	        			List<CourseElementInstance> $x_cei = manager().getCourseElementInstance(0, 1, eq(CourseElementInstance.CourseInstance, $x_ci.get(0).getId()));
		        		if($x_cei.size() > 0) {
		        			List<ProposedScheduling> $x_ps = manager().getProposedScheduling(0, 1, eq(ProposedScheduling.ElementInstance, $x_cei.get(0).getId()));
			        		if($x_ps.size() > 0) {
				                ...
				                put("lecturer", manager().getPerson(all(isNull(Person.Deleted), eq(Person.HomeDepartment, $p.getDepartment().getId()), eq(Person.IsLecturer, true))));
        ...
        if (schedulerManager.isReady()) {
            ...
            if ($role.schedulerstatus.equals("showConflicts")) {
                ...
                put("lecturerOverlap", schedulerManager.getScheduleScore($role.possibleProgram)
                        .getCourseConflictsLecturer());
    ...
    public void showCourseInstance(final String[] $path) throws DatabaseException {
        if ($path.length > 0) {
        	List<CourseElementInstance> $list_cei_from_ci = manager().getCourseElementInstance(all(eq(CourseElementInstance.CourseInstance, Parse.parseInt($path[0], -1))));
        	if($list_cei_from_ci.size() > 0) {
        		...
        		for(CourseElementInstance $cei : $list_cei_from_ci) {
        			List<ProposedScheduling> $tmp = manager().getProposedScheduling(1, all(eq(ProposedScheduling.ElementInstance, $cei.getId())));
        			if($tmp.size() > 0) {

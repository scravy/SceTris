...
<xsl:template match="vars:meta" mode="inner-builder">
	...
	<p>
		<input type="button" value="+"
			onclick="document.getElementById('features').appendChild(document.getElementById('features').lastElementChild.cloneNode(true));this.style.visibility = 'hidden'; " />
		<select class="unfancy" name="featureId">
		...
		<input type="text" class="text-input" value="0" name="featureQuantity" />
	</p>
	</div>
	
	<div id="noScriptFeatures">
		<xsl:for-each select="//item:features">
		<p class="formField">
			...
			<input id="feature{position()}" class="text-input input-numeric" type="text" name="featureQuantity" value="0" />
		</p>
		</xsl:for-each>
	</div>
	<script type="text/javascript">
		document.getElementById('noScriptFeatures').style.display = 'none';
	</script>

		...
		<div style="position: absolute;">
			...
			<xsl:for-each select="//vars:proposedScheduling/item:proposedScheduling">
				...
				<xsl:call-template name="pscell">
					...
					<xsl:with-param name="edit_path" select="'de.fu.scetris.web.forms.ShowTimetableByLecturer'"/>
					...
					<xsl:with-param name="hinttext">
						<xsl:value-of select="//vars:tpir/item:tpir[@id = current()/@room]/vars:building/item:building/@name"/>
						<xsl:text> 	</xsl:text>
						(<xsl:value-of select="//vars:tpir/item:tpir[@id = current()/@room]/vars:building/item:building/@address"/>)
						<xsl:text> 	Room: </xsl:text>
						<xsl:value-of select="//vars:tpir/item:tpir[@id = current()/@room]/@number"/>
					</xsl:with-param> 
				</xsl:call-template>
			</xsl:for-each>

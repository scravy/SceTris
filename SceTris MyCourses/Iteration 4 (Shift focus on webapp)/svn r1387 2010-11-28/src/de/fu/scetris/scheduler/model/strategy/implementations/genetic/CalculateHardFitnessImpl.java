...
public class CalculateHardFitnessImpl implements CalculateHardFitness {
	 ...
	 * <p>
	 * Hard fitness is the fitness that considers constraints which have to be fulfilled. If they
	 * are not fulfilled the courses cannot take place.
	 * <p>
	 * Rules of assignment:
	 * <p>
	 * (1) Each course has a scheduled score and a maximum score where the maximum score for each
	 ...
	 * <p>
	 * (2) If there is not more than 1 course in the same room at the same time: score + 1
	 * <p>
	 * (3) If the course lecturer has no other course at the same time: score + 1
	 * <p>
	 ...
	 * <p>
	 * As the fitness value is defined by the position of the course it is calculated by iterating
	 * over the course entries of the <code>Schedule</code> map.
	 * <p>
	 * First rule (2) and (3) will be tested. Second the constraints of a course will be checked one
	 * after another. In each iteration step the maximum score and the schedule score will be saved
	 * and added to the final fitness at the end of an iteration step.
	 * 
	...
	public double calculateHardFitness(final Schedule schedule,
		...
		for (Entry<CourseElementInstance,RoomTimeIndex> entry : ((ScheduleImpl) schedule)
			...
			for (ElementInstancePrefersTimeslot coursePrefersTimeSlot : course
				...
				if (coursePrefersTimeSlot.getPriority() == 100) {
					...
					int preferredTimeSlotIndex = config.timeSlotList.indexOf(coursePrefersTimeSlot
							.getTimeslot());
		...
		schedule.setTotalHardConstraintCount(maximumScore);
		schedule.setSatisfiedHardConstraintCount(scheduledScore);

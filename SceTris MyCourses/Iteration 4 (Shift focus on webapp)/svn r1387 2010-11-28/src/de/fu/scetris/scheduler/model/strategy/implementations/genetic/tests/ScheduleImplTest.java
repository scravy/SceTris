...
import de.fu.scetris.scheduler.model.strategy.implementations.genetic.RoomTimeCourseSlot;
...
public class ScheduleImplTest {
	...
	public void testInitializeAreConstraintsSatisfied()
		...
		for (Entry<CourseElementInstance, List<Boolean>> entry : schedule
			...
			int n = 0;
			n += entry.getKey().objectsOfElementInstancePrefersRoom()
					.size();
			n += entry.getKey().objectsOfElementInstancePrefersTimeslot()
					.size();
			n += entry.getKey().objectsOfElementInstanceRequiresFeature()
					.size();

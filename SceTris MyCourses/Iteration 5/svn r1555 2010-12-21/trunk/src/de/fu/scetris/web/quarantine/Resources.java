...
public class Resources extends FriggModule<RelationManager> {
	...
	public void editDepartment(final String[] path,
		...
		put("departments", getRelationManager().getDepartment(not(eq("id", data))));
		put("crud", "update");
		put("data", getRelationManager().getDepartment(data));
	}
	...
	public void editDepartmentCommit(final String[] target,
		...
		depart.setName(name);
		if (superDep > 0) {
			Department superDepartment = getRelationManager().getDepartment(superDep);
	...
	public void newDepartmentCommit(final String[] target,
		...
		if (!name.isEmpty()) {
			...
			if (superDepartment > 0) {
				tmp.setSuperordinateDepartment(getRelationManager().getDepartment(superDepartment));
			} else {
	...
	public void roomtimetable(final String[] target,
		...
		if (data > 0) {
			Room tmp = getRelationManager().getRoom(data);
			put("days", getRelationManager().getDay());
			put("proposedScheduling", getRelationManager()
					.getProposedScheduling(eq("room", tmp.getId())));
			put("metaceis", getRelationManager().getCourseElementInstance());
			put("timeslots", getRelationManager().getTimeslot());
			put("rooms", getRelationManager().getRoom());
			put("myroomfeature", tmp.whereSubjectOfRoomProvidesFeature());
		} else {
			Collection<Department> deps = getRelationManager().getDepartment();
			TreeMap<Integer,Integer> firstroomindeparment = new TreeMap<Integer,Integer>();
			int id = 0;
			for (Department x : deps) {
				Collection<Building> buildings = getRelationManager().getBuilding(eq("department",
																					 x.getId()));
				if (!buildings.isEmpty()) {
					id = buildings.iterator().next().getId();
					Collection<Room> rooms = getRelationManager().getRoom(eq("building", id));
					if (!rooms.isEmpty()) {
	...
	public void roomtimetableedit(final String[] target,
		...
		if ((oldtime > 0) && (newtime > 0)) {
			...
			Collection<ProposedScheduling> pss = getRelationManager()
			...
			if (!pss.isEmpty()) {
				...
				if (ps.getElementInstance().getDuration() + ((newtime - 1) % 12) <= 12) { /*
																						   ...
																						   * dayborder
																						   */
					ps.setTimeslot(getRelationManager().getTimeslot(newtime));
					ps.pushChanges();
		...
		put("days", getRelationManager().getDay());
		put("proposedScheduling", getRelationManager().getProposedScheduling(eq("room", data)));
		put("metaceis", getRelationManager().getCourseElementInstance());
		put("timeslots", getRelationManager().getTimeslot());
		put("rooms", getRelationManager().getRoom());
	}

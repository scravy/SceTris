...
public class Coursemanagement extends FriggModule<RelationManager> {
	...
	public Coursemanagement(final Scetris parent) {
		super(parent);
	}
	...
	public void newCEImStep1(final String[] path)
			throws DatabaseException {
		put("data", getRelationManager().getCourseInstance());
		put("crud", "create");
	}

	@Commit(action = "courseelementinstancenew", after = "courseelementinstancemeta")
	public void newCEIStep1Commit(final String[] target,
		...
		if ((courseinstance > 0) && (numberofelements > 0)) {
			CourseInstance ci = getRelationManager().getCourseInstance(courseinstance);
			Collection<CourseElement> ces = getRelationManager().getCourseElement(eq("part_of",
																					 ci.getInstanceOf()
																							 .getId()));
			if (ces.isEmpty()) {
			...
			for (int i = 0; i < numberofelements; i++) {
				getRelationManager().createCourseElementInstance(ci, ce, 1);
			}
	...
	public void newCEIStep2(final String[] path,
		...
		put("courseelementinstance",
			getRelationManager().getCourseElementInstance(eq("course_instance", data)));
		put("courseelements",
			getRelationManager().getCourseElement(eq("part_of", tmp.getInstanceOf().getId())));
		put("crud", "update");
	}

	@Commit(action = "courseelementinstancemeta", after = "courseelementinstancelist")
	public void newCEIStep2Commit(final String[] target,
	...
	public void newCourseInstanceCommit(final String[] target,
		...
		if ((course > 0) && (program > 0) && (mainlecturer > 0)) {
			...
			if (getRelationManager().getCourseInstance(all(eq("program", p.id()),
														   eq("instance_of", c.id())))
					.isEmpty()) {
	...
	public void newProgramStep1Commit(final String[] target,
		...
		if ((academicterm > 0) && (department > 0) && (pmanager > 0)) {
			...
			Program tmp = getRelationManager().fullyNewProgram(at, d, false, false, p);
			tmp.create();
	...
	public void newProgramStep2(final String[] path,
		...
		put("courses", getRelationManager().getCourse());
		put("program", getRelationManager().getProgram(pnum));
		put("crud", "create");
	}
	...
	public void newProgramStep2Commit(final String[] target,
					...
					throws DatabaseException {
		if (program > 0) {
			Program tmp = getRelationManager().getProgram(program);

			/* booleancheck */
			for (String x : courses) {
				int coursenum = Integer.parseInt(x);
				Course cx = getRelationManager().getCourse(coursenum);
				CourseInstance cix = getRelationManager().newCourseInstance(tmp,
																			cx,
																			tmp.getAcademicTerm()
																					.getStartLessons(),
																			tmp.getAcademicTerm()
	...
	public void newProgramStep3(final String[] path,
		...
		put("persons", getRelationManager().getPerson()); /* TODO :: ONLY LECTURER */
		put("crud", "create");
	}

	@Commit(action = "programmeta", after = "programlist")
	public void newProgramStep3Commit(final String[] target,
		...
		for (int i = 0; i < ids.length; i++) {
			...
			ci.setMainLecturer(getRelationManager().getPerson(Integer.parseInt(mainlecturer[i])));
			ci.pushChanges();
		}

...
public class ActionReflector<M extends Module<? extends RelationManager>> {
	...
	private ParamReflector[] commitArguments = null;
			...
			if ($arguments[$i] instanceof ArgReflector) {
				...
				if ($form.getClass().getCanonicalName().equals($formType) && ($formName != null)) {
					for (String $fieldName : $reflector.getFieldNames()) {
						...
						try {
							$reflector.setField($form, $fieldName, $paramArray);
	...
	void setCommitMethod(final Method method) throws ModuleException {
		if (method == null) {
			return;
		}
		Commit commitAnnotation;
		if ((commitAnnotation = method.getAnnotation(Commit.class)) != null) {
			afterCommit = commitAnnotation.after();
			analzyeCommitMethod(method);
			commitArguments = analyzeMethod(method);
		} else {
			afterCommit = null;

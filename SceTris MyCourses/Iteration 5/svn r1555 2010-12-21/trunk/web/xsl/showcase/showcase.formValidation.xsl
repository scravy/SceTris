...
<xsl:template mode="title" match="vars:meta">
	<xsl:value-of select="$mod-lang/formValidation/title" />
</xsl:template>

<xsl:template mode="content" match="vars:meta">
	<h1><xsl:value-of select="$mod-lang/formValidation/heading" /></h1>
	<p>
		<xsl:value-of select="$mod-lang/formValidation/description" />
	</p>
	...
	<xsl:call-template name="form-builder">
		...
		<xsl:with-param name="mixin" select="document('')/*/form:szimpla/*" />
		<!-- in this case, the mixed in stuff is taken from the stylesheet itself
		     via document("") - which refers to the current file. See //form:szimpla -->
	</xsl:call-template>
</xsl:template>

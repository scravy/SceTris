...
import de.fu.weave.util.StringGenerator;
...
abstract public class BaseImporter<T> implements Importer<T> {
	 ...
	 * 
	 */
	final protected BlockingQueue<Object> messages = new LinkedBlockingQueue<Object>(16);
	...
	protected List<Class<?>> initDependencies(final Map<Class<?>,Object> resources) {
		...
		List<Class<?>> avail = new LinkedList<Class<?>>();

		ClassesComparator cc = ClassesComparator.comparator;
		Map<Class<?>,Method> relationCreators = new TreeMap<Class<?>,Method>(cc);

		Method[] methods = manager.getClass().getMethods();
		for (Method m : methods)
			if (m.getName().startsWith("new")) relationCreators.put(m.getReturnType(), m);
		...
		resources.put(String.class, new StringGenerator());

		for (Class<?> c : avail)
			if (!resources.containsKey(c)) try {
		...
		while (!needs.isEmpty()) {
			...
			outer: for (Class<?> relation : currentNeeds) {
				...
				for (Class<?> param : params)
					if (!avail.contains(param)) continue outer;
				avail.add(relation);
		...
		for (Iterator<Class<?>> it = avail.iterator(); it.hasNext(); clazz = it.next())
			if (!relationCreators.containsKey(clazz)) list.add(clazz);

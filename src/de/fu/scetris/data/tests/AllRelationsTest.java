...
public class AllRelationsTest extends DatabaseTestSkeleton {
    ...
    protected Object[] makeValues(final Method m) {
        ...
        for (Class<?> p : params) {
            ...
            if (values[i] instanceof ValueGenerator) {
                values[i] = ((ValueGenerator<?>) values[i]).generate();
            }
            i++;
        }
        return values;
    ...
    public void testAllFullyCreate() throws Exception {
        ...
        loop: for (Method m : manager.getClass().getDeclaredMethods()) {
            if (m.getName().startsWith("fullyCreate")) {
                if (Relationship.class.isAssignableFrom(m.getReturnType())) {
                    continue loop;
                }
                try {
                    ...
                    Exception $e = (Exception) e.getCause();

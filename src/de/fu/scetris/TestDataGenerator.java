...
public class TestDataGenerator {
    ...
    public static void main(final String... args) throws DatabaseException,
        ...
        double percentageOfSoftConstraints = 0.25;
        double percentageOfHardConstraints = 0.50;
        double percentLecturer = 0.2;
        double percentStudents = 0.7;
        double percentTutors = 0.1;
        ...
        if (args.length > 0) {
            ...
            for (int i = 0; i < args.length; ++i) {
                ...
                if (args[i].equals("-pl")) {
                    percentLecturer = Integer.parseInt(args[i + 1]) / 100.0;
                }

                if (args[i].equals("-ps")) {
                    percentStudents = Integer.parseInt(args[i + 1]) / 100.0;
                }

                if (args[i].equals("-pt")) {
                    percentTutors = Integer.parseInt(args[i + 1]) / 100.0;
                }
            }
        }
        ...
        System.out.println("Percent of lecturer: " + percentLecturer * 100);
        System.out.println("Percent of students: " + percentStudents * 100);
        System.out.println("Percent of tutors: " + percentTutors * 100);
        ...
        test.generateAllPresentData(numberOfAcademicTerms, numberOfDepartments,
                ...
                percentageOfSoftConstraints, percentLecturer, percentStudents, percentTutors);
	...
	private List<CourseInstance> courseInstanceList = new ArrayList<CourseInstance>();
    ...
    public TestDataGenerator(final Random random,
        ...
        departmentToLecturer = new TreeMap<Department,List<Person>>();
        departmentToTutors = new TreeMap<Department,List<Person>>();
        departmentToStudents = new TreeMap<Department,List<Person>>();

        programList = new ArrayList<Program>();
        builingList = new ArrayList<Building>();
        roomList = new ArrayList<Room>();
        featureList = new ArrayList<Feature>();
        courseElementTypeMap = new TreeMap<String,CourseElementType>();
        courseNameToCourse = new TreeMap<String,Course>();
        courseElementList = new ArrayList<CourseElement>();
        courseInstanceMap = new TreeMap<Program,Map<Course,CourseInstance>>();
    ...
    public void generateAllPresentData(final int numberOfAcademicTerms,
                                       ...
                                       final int numberOfRoomsPerBuilding,
                                       final double percentageOfHardConstraints,
                                       final double percentageOfSoftConstraints,
                                       final double percentLecturer, final double percentStudents,
                                       final double percentTutors)
            throws DatabaseException,
            IOException, SAXException, ParserConfigurationException {
        ...
        generatePersons(personDataList.size(), percentLecturer, percentStudents, percentTutors);
        ...
        dayList = relationManager.getDay();
        timeSlotList = relationManager.getTimeslot();
        timeSlotsPerDay = Configuration.countTimeSlotsPerDay(timeSlotList);
    ...
    public void generateCourses(final Department department,
                                final int numberOfCourses, final int numberOfLecturerPerCourseMin,
        ...
        for (String courseData : courseDataList) {
            ...
            if (course == null) {
                course = relationManager.createCourse(courseName);
            ...
            for (int i = firstAcademicTermIndex; i < academicTermList.size(); i += repetition) {
                ...
                if (courseToCourseInstance == null) {
                    courseToCourseInstance = new TreeMap<Course,CourseInstance>();
                ...
                for (int j = 0; j < numberOfCoursesToOffer; ++j) {
                    ...
                    remainingCoursesForRoomPreference.add(courseElementInstance);
                    remainingCoursesForTimeSlotPreference.add(courseElementInstance);
                    remainingFeaturesPerCourse.put(courseElementInstance, new ArrayList<Feature>(
                            featureList));
...
//		int percentageOfLecturers = (int) (lecturerList.size() * percentage);
        ...
        for (int i = 0; (i < percentageOfCourses)
            ...
            while (!room.getBuilding().getDepartment()
                    .equals(courseElementInstance.getCourseInstance().getProgram().getDepartment())) {
    ...
    public void generatePersons(int numberOfPersons,
        ...
        for (int i = 0; i < numberOfPersons; ++i) {
            ...
            person.setHomeDepartment(currentDepartment);
            person.pushChanges();
    ...
    public void generatingRooms(final int numberOfRoomsPerBuilding,
        ...
        for (Building building : builingList) {
            ...
            for (int i = 0; i < numberOfLectureHallPerBuilding; ++i) {
                Room room = intermediateRoomList.get(i);
                relationManager.createRoomProvidesFeature(
                        room, featureList.get(0), 200);
                room.setName("LH" + room.getName());
                room.pushChanges();
            }
            ...
            for (int i = numberOfLectureHallPerBuilding; i < intermediateRoomList
                ...
                Room room = intermediateRoomList.get(i);
                relationManager.createRoomProvidesFeature(
                        room, featureList.get(0), 50);
                room.setName("R" + room.getName());
                room.pushChanges();
            }

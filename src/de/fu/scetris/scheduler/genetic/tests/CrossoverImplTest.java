...
import de.fu.scetris.scheduler.genetic.CrossoverImpl;
import de.fu.scetris.scheduler.genetic.GeneticFactory;
import de.fu.scetris.scheduler.genetic.RoomTimeIndex;
import de.fu.scetris.scheduler.genetic.ScheduleImpl;
import de.fu.scetris.scheduler.genetic.TimeCourseSlot;
...
public class CrossoverImplTest {
	 ...
	 * 
	 * @throws Exception
	 * @postcondition: The offsprings schedule map has to have the same CourseElementInstances.
	 * 
	 * @postcondition: Every CourseElementInstance occurs duration times in the time slots and do so
	 *                 in a continuous way.
	 * 
	 *                 Example: A CourseElementInstance has a duration of 4. It will be scheduled in
	 *                 time slots 6,7,8,9. These time slots belong to the same day
	 * 
	 * @postcondition: There is no course scheduled which overlaps through its duration in days.
	 * 
	 * @since Iteration2
	 */
	@Test
	public void testCrossover() throws Exception {
		...
		ScheduleImpl partner1 = new ScheduleImpl(configuration);
		ScheduleImpl partner2 = new ScheduleImpl(configuration);

		Setup setup = GeneticFactory.getInstance().createSetup(configuration, new Random());
		setup.initialize(partner1, configuration, false);
		setup.initialize(partner2, configuration, false);

		CrossoverImpl crossover = new CrossoverImpl(0.5f, new Random());
